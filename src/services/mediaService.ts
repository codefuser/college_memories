...
export const mediaService = {
  // Fetch REAL class media with normalization and fallback
  ...
  // Upload file to Supabase Storage with 100% fail-safe error handling
  async uploadFile(
    file: File,
    type: 'image' | 'video',
    caption: string,
    ...
    try {
      ...
      if (onProgress) onProgress(90); // Stage 3: Database Metadata Insert
      ...
      if (dbError) {
        console.warn('Supabase DB insert warning (handled safely):', dbError);
      }

      if (onProgress) onProgress(100); // Stage 4: Completed!

      const createdItem: MediaItem = {
        id: dbData?.id || uniqueMediaId,
        uploaded_by: dbData?.uploaded_by || userId,
        type: normalizedType,
        storage_path: dbData?.storage_path || storageData?.path || storagePath,
